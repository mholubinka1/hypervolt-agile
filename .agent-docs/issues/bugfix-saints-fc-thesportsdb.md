...
- [ ] Given `eventsnext.php` returns an event with today's `dateEventLocal`, when the poll runs,
      then a `LedTheme` with `effect_name="saints_fc_matchday"` and the alternating red/white
      51-element `leds` array is cached
- [ ] Given `eventsnext.php` has nothing for today but `eventslast.php` returns an event with
...
- [ ] Given neither endpoint has an event dated today, when the poll runs, then `resolve(now)`
      returns `None`
- [ ] Given either endpoint's response has `"events": null` or `"results": null` (no fixtures in
      that window), when the poll runs, then it's treated as no fixtures, not a crash
- [ ] Given `api_key` is omitted from `config:`, when the extension is constructed, then it
      defaults to `"3"` (TheSportsDB's free shared key)
- [ ] Given `team_id` is omitted from `config:`, when the extension is constructed, then it
      defaults to `134778` (Southampton FC's TheSportsDB ID)
- [ ] Given a poll fails (network error, non-2xx response), when the poll runs, then a warning is
      logged and the previously-cached match-day value is left unchanged (unchanged from the
      existing behaviour)
- [ ] `resolve(now)` still performs no I/O itself — cached-value read only (unchanged)
- [ ] `tests/hypervolt/test_led_load_shipped_extensions.py`'s existing integration test (loads the
      real shipped file via the real `load_extensions` path) is updated to mock TheSportsDB's
      response shape and still passes
- [ ] `config.yml.template`'s `saints_fc` example documents the new default `team_id` (134778)
