...
"no I/O in `__init__`" guideline: the file's content is knowable at startup, it is a single
small YAML, and reading it in the constructor is what turns a bad file into a clean "extension
absent" outcome (ADR 0007) instead of a failure re-hit on every poll. The alternative — a lazy
first-use load inside `resolve()` — would make a static, startup-knowable problem into a
recurring runtime one. ADR 0006's "self-contained" is narrowed to its
