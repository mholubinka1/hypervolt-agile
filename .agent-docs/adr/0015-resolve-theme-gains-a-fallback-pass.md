...
copy (deep-copied `leds`, `always_on` carried through, `active_until` populated — carried
from the fallback theme on this pass), so both passes return the same shape.
